...
foreach($pages as $key => $obj) {
    echo "Processing ".$key.": ";
    if (!is_data_valid($obj)) { // check for valid width/height and image
...
function process_info ($obj, $imagePath, $audioPath){
	$template 	 	= file_get_contents("template/audio.html");
	$linkTemplate 	= file_get_contents("template/item.html");
	
	$info = "";
	$links = "";
	foreach($obj as $key => $props) {
		default_if_nonexisting("title", $props, "TITLE");
		default_if_nonexisting("audio", $props, "invalid.mp3");
		...
		$fname = "template/svg/".sanitize_path($props["selector"].".svg_part");
		
		if (file_exists($fname) === true) {
            $selector = file_get_contents($fname);
        } else {
            $selector = "";
        }
		
		$tmp = str_replace("%ITEM%", $key, $tmp);
		$tmp = str_replace("%SELECTOR%", $selector, $tmp);
